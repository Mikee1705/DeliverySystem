...
// 1. OPEN THE MAIN LOGIC BLOCK
if (isset($_POST["add_customer"]) || isset($_POST["Update"])) {

    // 2. DEFINE VARIABLES (Now they exist for the rest of this block)
    ...
    if (empty($errors)) {
        $stmt = null;
        if ($cust_id) {
            $stmt = $conn->prepare("SELECT CUST_ID FROM CUSTOMER WHERE CUST_NAME = ? AND CUST_ID <> ?");
            $stmt->bind_param("si", $name, $cust_id);
        } else {
            $stmt = $conn->prepare("SELECT CUST_ID FROM CUSTOMER WHERE CUST_NAME = ?");
    ...
    // --- CONTACT VALIDATION (Fixed Structure + Added '09' Check) ---
    if ($contact === '') {
    ...
    if (empty($errors)) {
        $stmt = null;
        if ($cust_id) {
            $stmt = $conn->prepare("SELECT CUST_ID FROM CUSTOMER WHERE CUST_PHONE_NUMBER = ? AND CUST_ID <> ?");
            $stmt->bind_param("si", $contact, $cust_id);
        } else {
            $stmt = $conn->prepare("SELECT CUST_ID FROM CUSTOMER WHERE CUST_PHONE_NUMBER = ?");
    ...
    if (!empty($errors)) {
        ...
        if (isset($_POST['Update'])) {
            $stmt = $conn->prepare("UPDATE CUSTOMER SET CUST_NAME = ?, CUST_PHONE_NUMBER = ?, CUST_LOCATION = ? WHERE CUST_ID = ?");
            $stmt->bind_param("sssi", $name, $contact, $location, $cust_id);
            ...
            $stmt = $conn->prepare("INSERT INTO CUSTOMER (CUST_NAME, CUST_PHONE_NUMBER, CUST_LOCATION, CUST_ORDER) VALUES (?, ?, ?, ?)");
